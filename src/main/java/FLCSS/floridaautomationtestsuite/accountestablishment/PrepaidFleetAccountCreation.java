 ...
 * THEN Prepaid Fleet account is created successfully
...
@ClearSkyTest (
  id = "",
  scenario = "Prepaid Fleet account creation"
)
public class PrepaidFleetAccountCreation extends AbstractClearSkyTest {

  /**
   * The {@code Logger} to use in cooperation with all test instances.
   */
  private static final Logger LOG = LoggerFactory.getLogger(PrepaidFleetAccountCreation.class);
  ...
  @Test (
    ...
    description = "THEN Prepaid Fleet account is created successfully"
  )
  @ClearSkyTestMethod (stepNumber = 1)
  public void THEN_Prepaid_Fleet_account_is_created_successfully_0001(BaseUrl baseUrl, DataTable dataTable, Settings settings) {
    LOG.info("Test Execution : Scenario[Prepaid Fleet account creation], Step[0 : THEN Prepaid Fleet account is created successfully]");
   ...
   */
  @Test (
    dependsOnMethods = "THEN_Prepaid_Fleet_account_is_created_successfully_0001",
    dataProvider = "getStepParameters",
    dataProviderClass = ClasspathDataProvider.class,
  ...
  public void WHEN_We_enter_the_required_fields_and_create_the_account_0002(BaseUrl baseUrl, DataTable dataTable, Settings settings) {
    LOG.info("Test Execution : Scenario[Prepaid Fleet account creation], Step[1 : WHEN We enter the required fields and create the account]");
    throw new NotYetImplementedException();
  }
}
