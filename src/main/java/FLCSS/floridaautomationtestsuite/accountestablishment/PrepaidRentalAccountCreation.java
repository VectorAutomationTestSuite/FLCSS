 ...
 * THEN Prepaid Rental account is created successfully
 * </pre>
 */
@Test (
...
@ClearSkyTest (
  id = "",
  scenario = "Prepaid Rental account creation"
)
public class PrepaidRentalAccountCreation extends AbstractClearSkyTest {

  /**
   * The {@code Logger} to use in cooperation with all test instances.
   */
  private static final Logger LOG = LoggerFactory.getLogger(PrepaidRentalAccountCreation.class);
  ...
  public void WHEN_We_enter_the_required_fields_and_create_the_account_0001(BaseUrl baseUrl, DataTable dataTable, Settings settings) {
    LOG.info("Test Execution : Scenario[Prepaid Rental account creation], Step[0 : WHEN We enter the required fields and create the account]");
  ...
  @Test (
    ...
    dataProvider = "getStepParameters",
    dataProviderClass = ClasspathDataProvider.class,
    description = "THEN Prepaid Rental account is created successfully"
  )
  @ClearSkyTestMethod (stepNumber = 2)
  public void THEN_Prepaid_Rental_account_is_created_successfully_0002(BaseUrl baseUrl, DataTable dataTable, Settings settings) {
    LOG.info("Test Execution : Scenario[Prepaid Rental account creation], Step[1 : THEN Prepaid Rental account is created successfully]");
    throw new NotYetImplementedException();
  }
}
